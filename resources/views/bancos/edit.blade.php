            ...
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h2 class="font-bold text-2xl text-slate-800 tracking-tight leading-none">
                        {{ __('Editar Banco') }}: {{ $banco->nombre }}
                    </h2>
                    <p class="text-xs text-slate-500 mt-1">Modifica los datos de la entidad bancaria</p>
                </div>
            </div>
    ...
    <div class="py-6 space-y-6">
        <div class="w-full max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden p-6 sm:p-8">
                ...
                <form action="{{ route('bancos.update', $banco->id) }}" method="POST" class="space-y-6">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <select name="tipo_cuenta" id="tipo_cuenta"
                                    class="w-full text-sm bg-slate-50 border border-slate-300 rounded-xl px-4 py-2.5 focus:bg-white focus:border-cyan-500 focus:ring-2 focus:ring-cyan-200 transition-all font-medium text-slate-800">
                                <option value="Corriente" {{ old('tipo_cuenta', $banco->tipo_cuenta) == 'Corriente' ? 'selected' : '' }}>Cuenta Corriente</option>
                                <option value="Ahorros" {{ old('tipo_cuenta', $banco->tipo_cuenta) == 'Ahorros' ? 'selected' : '' }}>Cuenta de Ahorros</option>
                                <option value="POS / Datáfono" {{ old('tipo_cuenta', $banco->tipo_cuenta) == 'POS / Datáfono' ? 'selected' : '' }}>Terminal POS / Datáfono</option>
                            </select>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="text" name="titular" id="titular" value="{{ old('titular', $banco->titular) }}"
                                   class="w-full text-sm bg-slate-50 border border-slate-300 rounded-xl px-4 py-2.5 focus:bg-white focus:border-cyan-500 focus:ring-2 focus:ring-cyan-200 transition-all font-medium text-slate-800">
                        ...
                        <div>
                            ...
                            <select name="estado" id="estado" required
                                    class="w-full text-sm bg-slate-50 border border-slate-300 rounded-xl px-4 py-2.5 focus:bg-white focus:border-cyan-500 focus:ring-2 focus:ring-cyan-200 transition-all font-medium text-slate-800">
                                <option value="Activo" {{ old('estado', $banco->estado) == 'Activo' ? 'selected' : '' }}>Activo</option>
                                <option value="Inactivo" {{ old('estado', $banco->estado) == 'Inactivo' ? 'selected' : '' }}>Inactivo</option>
                            </select>
