            ...
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h2 class="font-bold text-2xl text-slate-800 tracking-tight leading-none">
                        {{ __('Gestión de Bancos y Cuentas') }}
                    </h2>
                    <p class="text-xs text-slate-500 mt-1">Configuración de entidades bancarias para transferencias, cheques y depósitos</p>
                </div>
            </div>

            <div class="flex items-center gap-2">
                <a href="{{ route('tarjetas.index') }}" class="inline-flex items-center gap-2 px-4 py-2.5 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-xl text-xs font-bold transition-all shadow-sm">
                    <svg class="w-4 h-4 text-slate-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 10h18M7 15h1m4 0h1m-7 4h12a3 3 0 003-3V8a3 3 0 00-3-3H6a3 3 0 00-3 3v8a3 3 0 003 3z"></path>
                    </svg>
                    Ver Tarjetas / POS
                </a>
                <a href="{{ route('bancos.create') }}" class="inline-flex items-center gap-2 px-4 py-2.5 bg-cyan-600 hover:bg-cyan-700 text-white rounded-xl text-xs font-bold shadow-md shadow-cyan-200 transition-all">
        ...
        <div class="w-full max-w-[1700px] mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div class="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm flex items-center justify-between">
                    <div>
                        <span class="text-xs font-bold uppercase tracking-wider text-slate-400">Total Bancos</span>
                ...
                <div class="bg-white p-5 rounded-2xl border border-emerald-100 shadow-sm flex items-center justify-between bg-emerald-50/20">
                    <div>
                        <span class="text-xs font-bold uppercase tracking-wider text-emerald-600">Bancos Activos</span>
                ...
                <div class="bg-white p-5 rounded-2xl border border-rose-100 shadow-sm flex items-center justify-between bg-rose-50/20">
                    <div>
                        <span class="text-xs font-bold uppercase tracking-wider text-rose-500">Bancos Inactivos</span>
            ...
            <div class="bg-white p-4 rounded-2xl border border-slate-200/80 shadow-sm">
                <form method="GET" action="{{ route('bancos.index') }}" class="flex flex-wrap items-center gap-3">
                    ...
                    <div class="relative flex-1 min-w-[260px]">
                        ...
                        <input type="text" name="buscar" value="{{ request('buscar') }}" 
                               placeholder="Buscar por nombre, código, cuenta o titular..." 
                               class="w-full pl-10 pr-4 py-2.5 text-xs bg-slate-50 border border-slate-200 rounded-xl focus:bg-white focus:border-cyan-500 focus:ring-2 focus:ring-cyan-100 font-medium text-slate-800 transition-all placeholder-slate-400">
                    </div>

                    <!-- Filtro Estado -->
                    <div class="min-w-[160px]">
                        <select name="estado" class="w-full text-xs bg-slate-50 border border-slate-200 rounded-xl py-2.5 px-3 focus:bg-white focus:border-cyan-500 focus:ring-2 focus:ring-cyan-100 font-medium text-slate-700 transition-all cursor-pointer">
                    ...
                    <!-- Botones de Acción -->
                    <div class="flex items-center gap-2">
                        <button type="submit" class="inline-flex items-center gap-1.5 px-4 py-2.5 bg-cyan-600 hover:bg-cyan-700 active:bg-cyan-800 text-white rounded-xl text-xs font-bold transition-all shadow-sm shadow-cyan-200">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 4a1 1 0 011-1h16a1 1 0 011 1v2.586a1 1 0 01-.293.707l-6.414 6.414a1 1 0 00-.293.707V17l-4 4v-6.586a1 1 0 00-.293-.707L3.293 7.293A1 1 0 013 6.586V4z"></path></svg>
                            <span>Filtrar</span>
                        </button>
                        @if(request()->anyFilled(['buscar', 'estado']))
                            <a href="{{ route('bancos.index') }}" class="inline-flex items-center gap-1.5 px-3.5 py-2.5 bg-slate-100 hover:bg-slate-200 text-slate-600 rounded-xl text-xs font-semibold transition-all">
            ...
            <div class="bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden">
                <div class="p-6 pb-4 border-b border-slate-100 flex justify-between items-center">
                    <div>
                        <h3 class="text-base font-extrabold text-slate-800">Listado de Bancos Registrados</h3>
                        <p class="text-xs text-slate-400">Total mostrando: {{ $bancos->total() }} entidades bancarias</p>
                    </div>
                </div>
                    ...
                    <table class="w-full text-sm text-left text-slate-600 divide-y divide-slate-100">
                        <thead class="bg-slate-50 text-slate-700 text-xs uppercase font-extrabold tracking-wider">
                            <tr>
                                ...
                                <th class="py-3.5 px-6">Nombre del Banco</th>
                                <th class="py-3.5 px-6">Tipo / N° Cuenta</th>
                                <th class="py-3.5 px-6">Titular</th>
                                <th class="py-3.5 px-6 text-center">Tarjetas / POS</th>
                        ...
                        <tbody class="divide-y divide-slate-100 bg-white">
                            @forelse ($bancos as $banco)
                                <tr class="hover:bg-slate-50/80 transition-colors">
                                    <td class="py-4 px-6 font-mono text-xs font-bold text-slate-400">#{{ $banco->id }}</td>
                                    <td class="py-4 px-6">
                                        <div class="flex items-center gap-3">
                                            <div class="w-9 h-9 rounded-xl bg-cyan-50 border border-cyan-100 flex items-center justify-center font-black text-cyan-600 text-xs">
                                                {{ strtoupper(substr($banco->nombre, 0, 2)) }}
                                            </div>
                                            <div>
                                                <p class="font-bold text-slate-800 text-sm">{{ $banco->nombre }}</p>
                                                <p class="text-xs text-slate-400">Código: {{ $banco->codigo ?? 'N/A' }}</p>
                                            </div>
                                        </div>
                                    </td>
                                    <td class="py-4 px-6">
                                        <span class="inline-flex items-center gap-1 px-2.5 py-1 rounded-lg text-xs font-semibold bg-slate-100 text-slate-700">
                                            {{ $banco->tipo_cuenta ?? 'General' }}
                                        </span>
                                    </td>
                                    <td class="py-4 px-6 text-xs text-slate-600 font-medium">
                                        {{ $banco->titular ?? 'No especificado' }}
                                    </td>
                                    <td class="py-4 px-6 text-center">
                                        <span class="inline-flex items-center justify-center w-7 h-7 rounded-full text-xs font-bold bg-indigo-50 text-indigo-700 border border-indigo-200">
                                            {{ $banco->tarjetas_count }}
                                        </span>
                                    </td>
                                    <td class="py-4 px-6 text-center">
                                    ...
                                    <td class="py-4 px-6 text-right whitespace-nowrap">
                                        <div class="flex items-center justify-end gap-2">
                                            <a href="{{ route('bancos.edit', $banco->id) }}" class="p-1.5 bg-slate-100 hover:bg-cyan-600 text-slate-600 hover:text-white rounded-xl transition-all shadow-sm" title="Editar">
                                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
                                            </a>

                                            <form action="{{ route('bancos.destroy', $banco->id) }}" method="POST" onsubmit="return confirm('¿Estás seguro de eliminar este banco?');" class="inline m-0">
                                                @csrf
                                                @method('DELETE')
                                ...
                                <tr>
                                    <td colspan="7" class="py-12 text-center text-slate-400">
                                        <div class="flex flex-col items-center justify-center">
                                            ...
                                            <p class="text-sm font-semibold">No se encontraron bancos con los filtros aplicados.</p>
                                        </div>
                                    </td>
                                </tr>
                            @endforelse
