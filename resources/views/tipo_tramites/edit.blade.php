<x-app-layout>
            ...
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h2 class="font-bold text-2xl text-slate-800 tracking-tight leading-none">
                        Editar Tipo de Trámite: {{ $tipoTramite->nombre }}
                    </h2>
                    <p class="text-xs text-slate-500 mt-1">Modifica el diseño de la tarjeta o los campos del formulario</p>
                </div>
            </div>
    ...
    <div class="py-8 bg-slate-50 min-h-screen" x-data="formBuilder()">
        <div class="w-full max-w-[1700px] mx-auto sm:px-6 lg:px-10">
            ...
            <form action="{{ route('tipo-tramites.update', $tipoTramite->id) }}" method="POST" @submit="prepareSubmission">
                @csrf
                @method('PUT')
                        ...
                        <div class="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm space-y-5">
                            <div class="flex justify-between items-center pb-3 border-b border-slate-100">
                                ...
                                <label class="inline-flex items-center gap-2 cursor-pointer">
                                    <input type="checkbox" name="activo" value="1" {{ $tipoTramite->activo ? 'checked' : '' }} class="rounded border-slate-300 text-indigo-600 shadow-sm focus:ring-indigo-500">
                                    <span class="text-xs font-bold text-slate-700">Activo en el Hub</span>
                                </label>
                            </div>
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <input type="text" name="nombre" x-model="nombre" placeholder="Ej: Poder Especial, Visa..." 
                                ...
                                <div>
                                    ...
                                    <input type="text" name="descripcion" x-model="descripcion" placeholder="Ej: Trámites consulares..." 
                        ...
                        <div class="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm space-y-5">
                            <div class="flex justify-between items-center pb-3 border-b border-slate-100">
                                <div>
                                    ...
                                    <p class="text-xs text-slate-400 mt-0.5">Agrega o edita los campos del formulario</p>
                                </div>
                                    ...
                                    <div class="p-4 bg-slate-50 rounded-xl border border-slate-200 relative group transition-all hover:border-slate-300">
                                        ...
                                        <div class="grid grid-cols-1 sm:grid-cols-12 gap-3">
                                            ...
                                            <div class="sm:col-span-5">
                                                ...
                                                <input type="text" x-model="campo.label" @input="campo.name = generateSlug(campo.label)" placeholder="Ej: País de Destino..." 
    ...
    <!-- Alpine.js Lógica para Form Builder en Edición -->
    <script>
        function formBuilder() {
            let initialCampos = @json($tipoTramite->campos ?? []);
            initialCampos = initialCampos.map(c => {
                c.options = c.options || [];
                c.optionsString = c.options.join(', ');
                return c;
            });

            return {
                nombre: '{{ addslashes($tipoTramite->nombre) }}',
                descripcion: '{{ addslashes($tipoTramite->descripcion ?? '') }}',
                selectedGradient: '{{ $tipoTramite->color_gradient }}',
                selectedIcon: '{{ $tipoTramite->icono }}',
